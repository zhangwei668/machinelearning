...
class optStruct:
    ...
    Parameters：
        ...
        C - 松弛变量
        toler - 容错率
        kTup - 包含核函数信息的元组,第一个参数存放核函数类别，第二个参数存放必要的核函数需要用到的参数a觉得函数的下降速度，（高斯胖瘦）
    """
...
def testRbf(k1 = 1.3):
    ...
    Returns:
        无
    """
    dataArr,labelArr = loadDataSet('F:\machinelearning\SVM\\testSetRBF.txt')                        #加载训练集
    ...
    dataArr,labelArr = loadDataSet('F:\machinelearning\SVM\\testSetRBF2.txt')                         #加载测试集
    errorCount = 0
    datMat = np.mat(dataArr); labelMat = np.mat(labelArr).transpose()         
    m,n = np.shape(datMat)
    for i in range(m):
